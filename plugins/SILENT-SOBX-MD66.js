...
cmd({
  ...
  try {
    ...
    if (quot) {
      ...
      if (quot.message.audioMessage || quot.message.pttMessage) {
        let anu = await conn.downloadAndSaveMediaMessage(quot.message.audioMessage || quot.message.pttMessage)
        return conn.sendMessage(from, { audio: { url: anu }, mimetype: 'audio/mpeg' })
      }
    }
    ...
    if (mek.quoted.mtype === "viewOnceMessage") {
      ...
      } else if (mek.quoted.message.audioMessage || mek.quoted.message.pttMessage) {
        let anu = await conn.downloadAndSaveMediaMessage(mek.quoted.message.audioMessage || mek.quoted.message.pttMessage)
        conn.sendMessage(from, { audio: { url: anu }, mimetype: 'audio/mpeg' })
      }
